...
writer = SummaryWriter("/edward-slow-vol/Sketch2Model/sketch2model/sgan_noise")
...
netG_A2B = Generator(use_dropout=True).to(device)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        pred_fake_B, _ = netD_B(fake_B)
        loss_GAN_A2B = criterion_GAN(pred_fake_B, target_real * 0.9)
        ...
        loss_D_fake = criterion_GAN(pred_fake_B, target_fake + noise)
        ...
        loss_D_B_real = loss_D_real + loss_classification_B_real
        loss_D_B_real.backward()
        loss_D_B_fake = loss_D_fake + loss_classification_B_fake
        loss_D_B_fake.backward()
        ...
        if i % 1000 == 0:
            loss_dict = {
                ...
                'loss_D': (loss_D_A_fake + loss_D_A_real + loss_D_B_fake + loss_D_B_real).item()*0.5,
                ...
                'loss_D_GAN': ((loss_D_A_fake + loss_D_A_real + loss_D_B_fake + loss_D_B_real) - 
                        (loss_classification_B_fake + loss_classification_B_real + loss_classification_A_fake + loss_classification_A_real)).item()*0.5,
